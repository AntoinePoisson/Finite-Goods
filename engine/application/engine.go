...
func Apply(world domain.World, command domain.Command) Result {
	...
	// Event IDs make retries harmless, as a real webhook consumer would require.
...
func returnFromCheckout(world *domain.World, command domain.Command) error {
	...
	// Browser redirects are user-controlled and therefore never prove payment.
...
func confirmPayment(world *domain.World, command domain.Command) error {
	...
	// Only the verified-event path owns the transition from held inventory to sold inventory.
