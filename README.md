...
by the engine, not by a disabled button, not by a debounce, but by a domain rule that returns
...
- **Event IDs make retries harmless.** Every command carries a UUID. The engine ignores a command
  whose event ID it has already recorded, so a replayed webhook, a double-click, or a retry
