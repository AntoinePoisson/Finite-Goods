...
// trend only — lighthouse already owns the hard budgets
test.describe('performance', () => {
  test('the homepage paints and settles quickly', async ({ page }, testInfo) => {
    ...
    const timings = await page.evaluate(async () => {
      const firstContentfulPaint = await new Promise<number | null>((resolve) => {
        ...
        // bail if the browser never records a paint
        setTimeout(() => {
          observer.disconnect();
          resolve(null);
        }, 5000);
  ...
  test('a reservation round trip stays fast', async ({ page }, testInfo) => {
    ...
    // hover/focus fills the demo profile
    ...
    // wasm + worker + lock + idb, the whole round trip
