...
if (!input) {
  throw new Error(
    "Pass the deployed site URL as the first argument or through DEPLOYMENT_URL.",
  );
}
...
const referenced = [
  ...home.matchAll(/<(?:script|link)[^>]+(?:src|href)=["']([^"']+)["']/gi),
]
...
for (const url of new Map(referenced.map((url) => [url.href, url])).values()) {
  ...
  if (!response.ok)
    throw new Error(`Referenced asset ${url} returned ${response.status}.`);
}
...
if (!engine.ok)
  throw new Error(`The WebAssembly engine returned ${engine.status}.`);
if (engine.headers.get("content-type") !== "application/wasm") {
  throw new Error(
    `The engine is served as ${engine.headers.get("content-type")}, not application/wasm.`,
  );
}
...
if (!robots.includes("User-agent: *"))
  throw new Error("robots.txt is not the one the build emits.");
